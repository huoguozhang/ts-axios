...
const transformRequestFn0 = (axios.defaults.transformRequest as AxiosTransformer[])[0]
const transformResponseFn0 = (axios.defaults.transformResponse as AxiosTransformer[])[0]
...
describe('defaults', () => {
  ...
  test('should transform request json', () => {
    expect(transformRequestFn0({ foo: 'bar' })).toBe('{"foo":"bar"}')
  })
  ...
  test('should transform response json', () => {
    const data = transformResponseFn0('{"foo":"bar"}')
  ...
  test('should do nothing to response string', () => {
    expect(transformResponseFn0('foo=bar')).toBe('foo=bar')
  })
